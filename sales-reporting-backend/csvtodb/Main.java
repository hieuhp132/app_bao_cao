...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        String csvPath = "./csvtodb/files/data_example.csv";
        ...
        try(DataBaseConnection dbc = new DataBaseConnection()) {
            Main.printCharacter('*', Main.header().length() / 2);
                ...
                dbc.checkColumnDataTypes(tableName);    
            Main.printCharacter('*', Main.header().length() / 2);
            ...
            Main.printCharacter('*', Main.header().length() / 2);
				...
				FilesProcess filesProcess = new FilesProcess(dbc.getConnection());
				
				List<String[]> csvData = filesProcess.readCSV(csvPath);
				
				if(csvData != null) filesProcess.printData(csvData);
